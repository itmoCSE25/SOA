...
const GetMinIdCityButton: React.FC<ButtonsProps> = ({onCityUpdate }) => {
  const handleButtonClick = async () => {
    try {
      const response = await fetch(`http://localhost:8080/api/external/min/id`);
      if (!response.ok) {
  ...
  return (
    <Stack direction="row" spacing={2}>
      <Button variant="outlined" onClick={handleButtonClick}>Город c min id</Button>
    </Stack>
  );
}
export default GetMinIdCityButton;
